...
def pos_tag(pages):
    tags = nltk.pos_tag(nltk.word_tokenize(pages))    
    print ('\n'.join(tags))
    chunk(tags)
...
def read_pdf(pdf_file):
    ...
    while(forward_pointer < num_pages or backward_pointer >= 0):
        ...
        rounding_section_present, continued_in_next_page = is_rounding_section_in_page(pdf_reader.getPage(pointer))
        if rounding_section_present: # If rounding section is found, POS tag page
            if continued_in_next_page: # In case rounding section is continued in next page
                ...
                cur_page = pdf_reader.getPage(pointer)
                next_page = pdf_reader.getPage(pointer + 1) 
                if verify_rounding_section_present(cur_page, next_page):
                    pos_tag([cur_page, next_page])
                ...
                pos_tag([pdf_reader.getPage(pointer)])
                break # Break the loop because the rounding section has been found
